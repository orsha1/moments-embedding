...
std::vector<Structure> initEmbed(std::string inputname0, std::string inputname1, Settings settings)
{
    ...
    structure_pbc.neighbors = mapNeighbors(structure_pbc.distances, 3.25);
            ...
            for (int k = -n; k < n + 1; ++k)
            {
                ...
                for (int row = 0; row < nat; ++row)
                {
                    for (int col = 0; col < 3; ++col)
                    {
                        ...
                        {
                            displaced_coordinates[row][col] += displace_vector[ang][col] * image_t[col];
                        }
                        // Directly assign to the preallocated vector
                        new_coordinates[index][col] = displaced_coordinates[row][col];
                    }
